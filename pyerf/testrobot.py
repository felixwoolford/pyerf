...
class SensorimotorOptions:
    def __init__(self):
        ...
        self.SENSES = {  'ORANGE': 0,
                    'PURPLE': 1,
                    'BLACK': 2,
                    'WHITE': 3,
                    }

        self.ACTIONS = { 'MZ': lambda x: False,
        ...
        self.COLOURS = { 0: (255/255,117/255,26/255),
...
class Robot(robot.Robot):
    def __init__(self, world, pos = np.array([3,3]), bearing = 0, hist = None):
        ...
        self.pos = pos
        self.bonus_grid = np.stack(np.meshgrid(np.arange(0,self.world.size,self.world.n),np.arange(0,self.world.size,self.world.n)),2).reshape(self.world.r**2,2)
    ...
    def iterate_part1(self):
        if self.hist == None:
            ...
            if (self.prev_act == "ML" and act == "MR" or
                self.prev_act == "MR" and act == "ML" or
                self.prev_act == "MU" and act == "MD" or
                self.prev_act == "MD" and act == "MU" ):
    ...
    def sense(self):
        ...
        if self.partner != None and np.linalg.norm(self.circular_distance(self.partner.pos,self.pos)) < 2:
            self.s_ = 1
...
class Viz(visuals.BaseEnvVisual):
    def __init__(self, widget, sim):
        super().__init__(widget)
        self.sim = sim
        self.canvas.bgcolor='b'
        ...
        self.robot_marker = scene.Markers(pos=self.sim.robot.bonus_pos+0.5,
                                   face_color = self.sim.robot.SO.COLOURS[3], symbol='square',parent = self.view.scene)
        self.robot2_marker = scene.Markers(pos=self.sim.robot2.bonus_pos+0.5,
                                   face_color = 'r', symbol='square',parent = self.view.scene)
   
        self.grid = scene.visuals.Image(self.sim.arena.grid.T, interpolation='nearest', cmap=self.cm,
    ...
    def iterate(self):
        ...
        self.robot_marker.set_data(pos=self.sim.robot.bonus_pos+0.5,
                                    face_color = self.sim.robot.SO.COLOURS[3], size = 1, scaling=True, symbol='square',)
        self.robot2_marker.set_data(pos=self.sim.robot2.bonus_pos+0.5,
                                    face_color = 'r', size = 1, scaling=True, symbol='square',)
    ...
    def reset(self):
        self.iterate()
c = core.Core(fps=30, speed = 60, framesync = False)
c.experiment = Exp()
c.gui.add_sim_visual(Viz,c.experiment.simulation)
c.gui.add_sim_visual(Viz,c.experiment.simulation)
c.run()
